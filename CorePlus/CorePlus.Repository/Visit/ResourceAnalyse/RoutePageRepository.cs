...
{
    public class RoutePageRepository : VisitRepository<ResourceClassificationBannerEntity>
    {
        ...
        public override IQueryable GetDataOfGrid(int page, int rows, DateTime start, DateTime end, int tag, string title, out int recordCount)
        {
            ...
            var temp = from x in query
                       where string.IsNullOrEmpty(title) ? true : x.RefererUrl.Contains(title)
                       group x by x.RefererUrl into g
                       select new
                       {
                           RefererUrl = g.Key,
                           PV = g.Count(),
            ...
            return temp.OrderByDescending(x => x.RefererUrl).Skip((page - 1) * rows).Take(rows);
        }
    }
}