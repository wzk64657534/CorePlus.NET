...
function initTb(queryParam) {
    $('#dg').datagrid({
        ...
        columns: [[
            { field: 'Budget', title: '每日预算', sortable: true },
            { field: 'RegionTarget', title: '推广地域' },
            { field: 'Schedule', title: '推广时段' },
            { field: 'ExcludeIp', title: 'IP排除' },
            { field: 'NegativeWords', title: '否定关键词' },
            { field: 'ShowProb', title: '创意展现方式' },
        ...
        onDblClickRow: function (rowIndex, rowData) {
            var id = rowData.ID;
            changeTabByIndex(1);
            location.href = "/Adgroup/Index/" + id;
        },
        toolbar: [{
            id: 'btnAdd',
            text: '添加',
...
function addEvent() {
    //添加的表单出来。
    $("#frameAdd").attr("src", "/Campaign/Create/");
