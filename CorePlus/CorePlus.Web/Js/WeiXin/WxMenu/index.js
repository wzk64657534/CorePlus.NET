...
function InitListFirstMenu() {
    $.getJSON('/WxMenu/GetFirstMenu', function (data) {
        firstMenus = data;
    });
}

function GetTextFromWxFirstMenuList(v) {
    if (firstMenus) {
        for (var i = 0; i < firstMenus.length; i++) {
            var item = firstMenus[i];
            if (item.Value == v) {
                v = item.Text;
                break;
            }
...
var wxMenuType = [
{ "Selected": false, "Text": "链接", "Value": "view" },
{ "Selected": false, "Text": "点击", "Value": "click" }
];

//InitListWxMenuType();

function InitListWxMenuType() {
    $.getJSON('/WxMenu/GetWxMenuTypeList', function (data) {
        wxMenuType = data;
    });
}

function GetTextFromWxMenuTypeList(v) {
    if (wxMenuType) {
        for (var i = 0; i < wxMenuType.length; i++) {
            var item = wxMenuType[i];
            if (item.Value == v) {
                v = item.Text;
                break;
            }
        ...
        columns: [[
            ...
            { field: 'ID', title: '编号', sortable: true },
            {
                ...
                formatter: function (val, data, index) {
                    val = GetTextFromWxMenuTypeList(val);
                    return val;
                }
            },
            { field: 'KeyOrUrl', title: '键值/链接' },
            {
                field: 'HighId',
                title: '上级菜单',
                formatter: function (val, data, index) {
                    val = GetTextFromWxFirstMenuList(val);
                    return val;
                }
            }
        ]],
...
function editEvent() {
    ...
    $("#frameEdit").attr("src", "/WxMenu/Edit/" + id);
...
function postDeleteData(ids) {
    $.post("/WxMenu/RemoveServant", { Ids: ids }, function (data) {
        if (data == "ok") {
            //清除之前的选中的行。
