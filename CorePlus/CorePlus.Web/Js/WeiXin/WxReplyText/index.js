...
function InitListMatchType() {
    $.getJSON('/WxReplyText/GetMatchTypeList', function (data) {
        matchTypeList = data;
    });
}
...
function initTb(queryParam) {
    $('#dg').datagrid({
        ...
        url: '/WxReplyText/GetDataByPager',
...
function addEvent() {
    //添加的表单出来。
    $("#frameAdd").attr("src", "/WxReplyText/Add/");
...
function editEvent() {
    ...
    $("#frameEdit").attr("src", "/WxReplyText/Edit/" + id);
    //弹出修改的对话框。
...
function postDeleteData(ids) {
    $.post("/WxReplyText/RemoveServant", { Ids: ids }, function (data) {
        if (data == "ok") {
            //清除之前的选中的行。
